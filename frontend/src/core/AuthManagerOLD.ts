import { AppState, AuthState, MessageType } from '../shared/constants.js';
import { LoginUser, RegisterUser } from '../shared/types.js';
...
export class AuthManager {
	...
	private handleLoginSubmit(): void {
		...
		wsClient.registerCallback(MessageType.SUCCESS_LOGIN, async (raw: string) => {
			try {
		...
		wsClient.registerCallback(MessageType.LOGIN_FAILURE, (msg: string) => {
	...
	private handleRegisterSubmit(): void {
		...
		wsClient.registerCallback(MessageType.USER_EXIST, (msg: string) => {
			this.authState = AuthState.LOGGED_FAILED;
			uiManager.clearForm(this.registrationFields);
			alert(msg || 'User already exists. Please login instead.');
			setTimeout(() => {
				appStateManager.navigateTo(AppState.LOGIN);
			}, 500);
		});

		wsClient.registerCallback(MessageType.USERNAME_TAKEN, (msg: string) => {
			this.authState = AuthState.LOGGED_FAILED;
			uiManager.clearForm(this.registrationFields);
			alert(msg || 'Username is already taken. Please choose another one.');
		});

		wsClient.registerCallback(MessageType.ERROR, (msg: string) => {
			this.authState = AuthState.LOGGED_FAILED;
			uiManager.clearForm(this.registrationFields);
		...
		wsClient.registerCallback(MessageType.SUCCESS_REGISTRATION, (msg: string) => {
			this.authState = AuthState.LOGGED_IN;
